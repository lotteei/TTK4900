...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        #Ta inn flere videoer
        
        path = 'results/tracker/' # Saving path
        ...
        #video_out = cv2.VideoWriter(path + 'video00001.mp4', fourcc, fps, frame_size) #todo
        ...
        for video in videos:
            ...
            for frame_num in tqdm(range(0, frame_count)): # frame_count 900 frames is 1 min
                ...
                if frame_num_1 % (save_rate) == 0:
                    dictionary_pos['Frame' + str(frame_num_1)] = {}
                    dictionary_vel['Frame' + str(frame_num_1)] = {}
                    dictionary_acc['Frame' + str(frame_num_1)] = {}
                    
                    for key in dictionary.keys():                        
                        if len(dictionary[key]) == (save_rate):
                            pos_list = []
                            x_0, y_0, z_0, x_1, y_1, z_1, dx_0, dy_0, dz_0, dx_1, dy_1, dz_1 = kalman(dictionary[key])

                            # Velocity
                            velocity = np.sqrt(np.square(dx_1) + np.square(dy_1) + np.square(dz_1))

                            dictionary_vel['Frame' + str(frame_num_1)][key] = velocity
                            ...
                            if len(dictionary_velocity_tot[key]) == 2:
                                ...
                                dictionary_acc['Frame' + str(frame_num_1)][key] = acceleration
                            ...
                            dictionary_pos['Frame' + str(frame_num_1)][key] = pos_list
                    ...
                    # Saving velocity and acc per frame%15
