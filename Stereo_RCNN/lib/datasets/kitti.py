...
class kitti(imdb):
    ...
    def _load_image_set_index_new(self):
        ...
        if self.name == 'kitti_train':
            train_set_file = open('/content/gdrive/MyDrive/Stereo_RCNN/data/kitti/splits/train.txt', 'r')
            image_index = train_set_file.read().split('\n')
        elif self.name == 'kitti_val':
            val_set_file = open('/content/gdrive/MyDrive/Stereo_RCNN/data/kitti/splits/val.txt', 'r')
